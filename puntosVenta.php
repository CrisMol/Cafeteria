...
<?php
    $contador = 0;
    $puntosVenta = conseguir_puntos_venta($db);
    if(!empty($puntosVenta) && mysqli_num_rows($puntosVenta) >= 1):
        while ($puntoVenta = mysqli_fetch_assoc($puntosVenta)) :
            $contador++;
?>
<tr>
                                  <td><?=$contador?></td>
                                  <td><?=$puntoVenta['NOMBRE_PUNTO']?></td>
                                  <td align="center"><button type="button" class="btn btn-sm btn-outline-info" data-toggle="modal" data-target="#modalEditar<?=$puntoVenta['CODIGO']?>">Editar</button></td>
                              </tr><!-- Modal -->
<div class="modal fade" id="modalEditar<?=$puntoVenta['CODIGO']?>" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
              <form method="POST" action="guardarCambiosPunto.php">

                    <div class="form-group">
                      <label class="form-label" for="simpleinput">Nombre Punto Venta</label>
                      <input type="text" id="simpleinput" name="nombrePunto" value="<?=$puntoVenta['NOMBRE_PUNTO']?>" class="form-control" autocomplete="off" required>
                  </div>


            </div>
            <div class="modal-footer">
               <input name="idPunto" type="hidden" id="idPunto" value="<?=$puntoVenta['CODIGO']?>" />
...
?>
<?php echo isset($_SESSION['errores']) ? mostrar_error($_SESSION['errores'], 'nombrePunto') : ''; ?>
<?php echo isset($_SESSION['errores']) ? mostrar_error($_SESSION['errores'], 'idPunto') : ''; ?>
<?php echo isset($_SESSION['errores']) ? mostrar_error($_SESSION['errores'], 'nombrePuntoNuevo') : ''; ?>
<?php borrar_error();?> 
  ...
  <div class="modal fade" id="modalNuevoPuntoVenta" tabindex="-1" role="dialog" aria-hidden="true">
      <div class="modal-dialog" role="document">
          <div class="modal-content">
              <div class="modal-header">
                  <h4 class="modal-title">
                      Registro Nuevo Punto de Venta                  </h4>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true"><i class="fal fa-times"></i></span>
                  </button>
              </div>
              <div class="modal-body">
                <form method="POST" action="guardarNuevoPunto.php">

                      <div class="form-group">
                        <label class="form-label" for="simpleinput">Nombre Punto Venta</label>
                        <input type="text" id="simpleinput" name="nombrePuntoNuevo" placeholder="Digite nombre" class="form-control" autocomplete="off" required>
                    </div>
