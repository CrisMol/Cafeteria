...
<?php
$contador = 0;
    $compras_proveedores = conseguir_compras_proveedor($db);
...
$contador = 0;
    ...
    if(!empty($compras_proveedores) && mysqli_num_rows($compras_proveedores) >= 1):
        while($compra_proveedor = mysqli_fetch_assoc($compras_proveedores)):
            $contador++;
?>
<div class="modal fade" id="modalProductos<?=$compra_proveedor['CODIGO']?>" role="dialog">
      <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
              <div class="modal-header">
                  <h4 class="modal-title">
                      Factura Pedido #<?=$compra_proveedor['CODIGO']?>                 <small><?=$compra_proveedor['NOMBRE_PROVEEDOR']?></small>
                  </h4>
...
<?php
$contador = 0;
    $productos_proveedores = conseguir_productos_compra_proveedor($db, $compra_proveedor['ID_PROVEEDOR'], $compra_proveedor['FECHA']);
    if(!empty($productos_proveedores) && mysqli_num_rows($productos_proveedores) >= 1):
        while($producto_proveedor = mysqli_fetch_assoc($productos_proveedores)):
            $contador++;
?>
                            <tr>
                               <td><?=$contador?></td>
                                <td><?=$producto_proveedor['CODIGO_PRODUCTO']?></td>
                                <td><?=$producto_proveedor['DESCRIPCION_PRODUCTO']?></td>
                                <td><?=$producto_proveedor['CANTIDAD']?></td>
                                <td align="right"><?=$producto_proveedor['COSTO']?></td>
                            </tr>   
<?php
        endwhile;
    endif;
